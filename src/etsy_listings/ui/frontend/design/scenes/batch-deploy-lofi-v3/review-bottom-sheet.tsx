...
export const meta = {
  title: "Batch deployment — review · v3",
  viewport: "laptop",
  description:
    "Clickable batch flow: slow-plan loading state, compact overall stage strip with live counts, and the individual stage comparison in a bottom drawer.",
};
const page = {
  minHeight: "100vh",
  display: "flex",
...
function stateOf(row: Change, stageKey: string, phase: Phase, completed: number): StageState {
  ...
  if (phase === "planning" || phase === "review") return "queued";
...
function RunChecklist({
  ...
  return (
    <section data-testid="run-stage-checklist" style={{ marginTop: 24 }}>
      ...
      <div
        className="wf-overall-steps"
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(6, minmax(135px, 1fr))",
      ...
      >
        {runStages.map((stage, i) => {
          ...
          const runningItem = runningIndex >= 0 ? stage.items[runningIndex] : undefined;
          const overallState: StageState = runningItem
            ? "running"
          ...
          return (
            <div
              ...
              style={{
                ...
                background: runningItem ? "#fff8d5" : overallState === "done" ? "#f3f8f2" : "#fff",
              }}
            >
              <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <Check state={overallState} />
              ...
              {runningItem ? (
                ...
                <p style={{ margin: "7px 0 0", color: "#555", fontSize: 12.5 }}>
                  {overallState === "done"
                    ? "All listings done"
                    : `${stage.items.length} listing${stage.items.length === 1 ? "" : "s"} will run`}
                </p>
              )}
            </div>
          );
...
export default function BatchReviewBottomSheetWireframe() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setCompleted((count) => count + 1), 820);
    return () => window.clearTimeout(timer);
  }, [completed, phase]);
  ...
  if (phase === "planning") {
    return (
      <div style={page}>
        <style>{`@keyframes wf-pulse{0%,100%{opacity:.3}50%{opacity:1}} @keyframes wf-spin{to{transform:rotate(360deg)}} .wf-planning-pulse{animation:wf-pulse 1.2s ease-in-out infinite} @media(max-width:900px){.wf-sidebar{display:none!important}.wf-main{padding:24px!important}}`}</style>
        ...
        <main className="wf-main" style={main}>
          <button
            type="button"
            className="wf-button"
            data-goto="batch-deploy-lofi-v3/listings"
            style={button}
          >
            ← Back to listings
          </button>
          <div style={{ maxWidth: 680, margin: "110px auto 0", textAlign: "center" }}>
            <span
              aria-hidden="true"
              style={{
            ...
            />
            <p style={{ margin: "22px 0 0", color: "#555" }}>Planning all listings</p>
            <h1 style={{ margin: "7px 0", fontSize: 30 }}>Checking what will change…</h1>
            <p style={{ margin: "0 auto", color: "#444", maxWidth: 520 }}>
              Reading live Printify and Etsy state, then comparing it with every listing in this
              workspace. This may take a little while.
            </p>
            <div style={{ marginTop: 30, border: "1px solid #999", textAlign: "left" }}>
              {[
                "Reading 6 listing files",
                "Checking live product and listing state",
                "Building one reviewable plan",
              ].map((label, i) => (
                <div
                  key={label}
                  style={{
            ...
            <p style={{ marginTop: 14, color: "#777", fontSize: 13 }}>
              The prototype continues automatically when the fake plan is ready.
            </p>
          </div>
        </main>
      </div>
  ...
  return (
    <div style={page}>
      <style>{`.wf-button:hover,.wf-row:hover { background:#e5e5e5 !important; } @media(max-width:900px){.wf-sidebar{display:none!important}.wf-main{padding:24px!important}.wf-diff{grid-template-columns:1fr!important}.wf-overall-steps{grid-template-columns:repeat(2,minmax(150px,1fr))!important}.wf-overall-steps>div:nth-child(odd){border-left:none!important}.wf-overall-steps>div:nth-child(n+3){border-top:1px solid #999}}`}</style>
      <aside className="wf-sidebar" style={sidebar}>
        <strong>Listings</strong>
        <span>Dashboard</span>
        <strong>Listings</strong>
      ...
      <main className="wf-main" style={main}>
        ...
        <RunChecklist onOpen={show} phase={phase} completed={completed} />
