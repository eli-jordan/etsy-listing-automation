...
class Stage(Protocol[D, A, L]):
    ...
    *live* state: the upload ids a stage may skip re-shipping were written by
    ...
    # which is the opposite of optional. `build_plan` looks it up with
    # `getattr(stage, "snapshot", None)` and stores whatever it returns, or
    # `None`, on `StagePlan.snapshot` -- documented here because it is still
    # part of what a stage *may* be asked, just not part of what every stage
    # *must* answer.
