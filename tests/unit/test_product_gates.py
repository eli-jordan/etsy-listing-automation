...
def test_a_design_at_the_print_area_passes(tmp_path: Path) -> None:
    check_design_resolution(_design(tmp_path, (4200, 4800)), PROFILE)
...
def test_a_design_within_ten_percent_passes(tmp_path: Path) -> None:
    ...
    switched off."""
    check_design_resolution(_design(tmp_path, (4000, 4800)), PROFILE)
...
def test_a_design_larger_than_the_print_area_passes(tmp_path: Path) -> None:
    ...
    check_design_resolution(_design(tmp_path, (8400, 9600)), PROFILE)
...
def test_a_design_short_on_either_axis_is_refused(tmp_path: Path, size) -> None:
    with pytest.raises(DesignResolutionError):
        check_design_resolution(_design(tmp_path, size), PROFILE)
...
def test_the_refusal_names_the_size_it_wanted(tmp_path: Path) -> None:
    ...
    with pytest.raises(DesignResolutionError) as exc_info:
        check_design_resolution(_design(tmp_path, (120, 140)), PROFILE)
...
def test_a_design_without_an_alpha_channel_is_refused(tmp_path: Path) -> None:
    ...
    of ink on the shirt."""
    with pytest.raises(DesignResolutionError, match="alpha"):
        check_design_resolution(_design(tmp_path, (4200, 4800), mode="RGB"), PROFILE)


def test_a_missing_design_is_refused_by_name(tmp_path: Path) -> None:
    with pytest.raises(DesignResolutionError, match="nope.png"):
        check_design_resolution(tmp_path / "nope.png", PROFILE)
...
def test_an_unchanged_garment_passes() -> None:
    check_garment_unchanged(APPLIED, blueprint_id=706, print_provider_id=29)


def test_no_previous_apply_passes() -> None:
    """Nothing to have changed from. The first `apply` creates the product."""
    check_garment_unchanged(None, blueprint_id=706, print_provider_id=29)


def test_a_changed_blueprint_is_a_hard_error() -> None:
    ...
    with pytest.raises(GarmentChangedError) as exc_info:
        check_garment_unchanged(APPLIED, blueprint_id=6, print_provider_id=29)
...
def test_a_changed_print_provider_is_a_hard_error() -> None:
    with pytest.raises(GarmentChangedError) as exc_info:
        check_garment_unchanged(APPLIED, blueprint_id=706, print_provider_id=99)

    assert "29" in str(exc_info.value) and "99" in str(exc_info.value)


def test_both_changing_at_once_names_both() -> None:
    with pytest.raises(GarmentChangedError) as exc_info:
        check_garment_unchanged(APPLIED, blueprint_id=6, print_provider_id=99)
...
def test_real_copy_passes() -> None:
    check_copy_is_concrete(title="Take A Hike Tee", description="A retro sunset.")


@pytest.mark.parametrize("title", ["<generate>", "", "   "])
def test_an_unresolved_or_empty_title_is_refused(title: str) -> None:
    with pytest.raises(UnresolvedCopyError, match="title"):
        check_copy_is_concrete(title=title, description="A retro sunset.")


@pytest.mark.parametrize("description", ["<generate>", "", "   "])
def test_an_unresolved_or_empty_description_is_refused(description: str) -> None:
    with pytest.raises(UnresolvedCopyError, match="description"):
...
def test_the_refusal_says_why_a_product_needs_them() -> None:
    ...
    with pytest.raises(UnresolvedCopyError) as exc_info:
        check_copy_is_concrete(title="<generate>", description="x")
