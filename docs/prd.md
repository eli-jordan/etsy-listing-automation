...
- **Etsy videos** have no rank, no update and no order parameter — only
  ...
  included (#72, measured 2026-09-25).
...
design.png + listing.yaml
        ...
        └─► etsy        patch title/desc/tags/section/materials/renewal
                        ...
                        attach videos in gallery order (#72)
```
...
etsy-listings/
  ...
  .cache/                           # gitignored
    ...
    market/                         # market-informed SEO (#71)
      search/  stats/               # 7-day Etsy response caches
      snapshots/{listing-name}.json # latest market research per listing
  settings.yaml                     # optional tuning, e.g. market scoring weights
  prompts/                          # editable AI instructions
    seo.md                           # seeded only when absent
    brief.md                         # ditto — the design-brief drafting prompt
    market-queries.md                # ditto — brief → three Etsy search queries
  garment-profiles/
  ...
  common-media/                       # shared images and videos (#72)
...
media:                     # the Etsy gallery, in order (#72)
...
(#72). Etsy decides that shape more than we do: the first entry is the
...
**Every path in `listing.yaml` is a ref with one of two roots** (#73): no
...
### `settings.yaml` — optional tuning

Workspace-wide settings that aren't shop facts. Today it holds only the
market-informed SEO scoring weights (#71, [market-seo.md](market-seo.md#scoring)).
The file and every key in it are optional. A missing file or key falls back to
the packaged default, and `setup` doesn't write it.
...
**Prompts are the one deliberate exception to "never overwrites" (#71).**
`setup` seeds each packaged prompt (`seo.md`, `brief.md`, `market-queries.md`)
only when it is missing. Without a flag, it warns for each existing prompt that
differs from its packaged default, naming the file and the flag.
`setup --replace-prompts` overwrites every prompt in `prompts/` with its
packaged default and keeps the previous file as `<name>.md.bak`, replacing any
older `.bak`. It is opt-in, because a prompt the seller edited is seller
content. It exists because a workspace would otherwise never receive new
instructions, such as the market-data rules in `seo.md`.
...
- Every SEO proposal is **market-informed** (#71, [market-seo.md](market-seo.md)).
  A provider call turns the brief and design into three Etsy search queries.
  Read-only Etsy calls then find and score comparable listings, and the
  proposal receives a market-data block built from them. The market data
  drives **wording and phrase priority**, never what is true about the
  listing. A failed query extraction or Etsy search fails the request.
  Finding no comparable listings does not: the proposal goes ahead with a
  warning.
- AI work runs as one **AI run** per listing (#71): brief (when drafting),
  then market research, then proposal. It runs on the server, reports each
  step as it happens, and outlives the editor. Leaving or reloading the
  editor reattaches to it, and only **Cancel** stops it. The server writes
  the drafted brief into `listing.yaml` itself, and only while the saved
  brief is still empty. That single field is the one write an AI run makes to
  a listing file.
...
- Each provider call (brief, query extraction, proposal) has one 60-second
  deadline, including one same-provider repair for malformed output and the
  permitted fallback. The whole run is capped at 3 minutes (#71). Pressing
  **Cancel** terminates the subprocess tree and retains no proposal. A brief
  or market snapshot already written stays.
- The Etsy calls are read-only and go through the existing retry policy. They
  follow Etsy's own per-app limits, read from its rate-limit headers; no
  budget is stored. Their responses and the latest market snapshot per listing
  are cached under the gitignored `.cache/market/`.
- `prompts/seo.md`, `prompts/brief.md` and `prompts/market-queries.md` are
  seller-editable plain instruction text. Setup seeds each default only when
  absent. It replaces seller content only under the opt-in
  `setup --replace-prompts` (#71). The repository-root
...
**Videos sync through their own stage, and by a different mechanism** (#72),
...
- Every media entry resolves: render references name a colour the listing
  offers, file refs exist under one of the two roots (#73). ≤20 images and ≤2
  videos; the first entry is an image, and any video puts one at position 2
  (#72).
- **Every video is one Etsy documents as acceptable** (#72): `.mp4` or `.mov`,
...
| 4 | AI lifecycle | AI Mode is a saved-listing Details-tab editing aid. It obtains one temporary local proposal from Codex or Claude Code, and the seller directly accepts individual title, tag, or description-lead choices into ordinary autosave fields. It writes no generated file, lockfile, or remote state; `plan`/`apply` only see accepted concrete values. **Amended (#68):** attaching a design also drafts the listing `brief` and writes it straight into that field, then starts the SEO request automatically. The brief is a generation *input*, not one of the three SEO outputs the seller reviews, and drafting only ever fills an empty one — so the review gate on copy that reaches Etsy is unchanged, while the seller stops paying a two-minute wait for work that could already have run. **Amended (#71):** a proposal is now market-informed, so AI Mode also makes read-only Etsy calls and writes the gitignored `.cache/market/`. The work runs as a server-side AI run that outlives the editor and is stopped only by Cancel. The drafted brief is written by that run, under the listing's write lock, and only while the saved brief is empty. The 60-second deadline applies to each provider call, and the whole run is capped at 3 minutes. It still writes no generated file, lockfile or remote state, and nothing else in `listing.yaml`. |
...
| 12 | Media | Explicit per-listing list; full replace on change. **Amended (#72):** the list holds videos too, and is the gallery in order. |
| 13 | AI inputs | Vision + short brief; seller-editable `prompts/seo.md` and `prompts/brief.md`; hard proposal validation before the browser sees it. **Amended (#71):** plus `prompts/market-queries.md`, and a market-data block (a ranked phrase list and the top 8 comparable listings) appended to the proposal context. |
