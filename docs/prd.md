...
- The request has one 60-second deadline, including one same-provider repair
  ...
  retains no proposal.
- `prompts/seo.md` is seller-editable plain instruction text. Setup seeds its
  default only when absent, never replacing seller content. The repository-root
...
| 4 | AI lifecycle | AI Mode is a saved-listing Details-tab editing aid. It obtains one temporary local proposal from Codex or Claude Code, and the seller directly accepts individual title, tag, or description-lead choices into ordinary autosave fields. It writes no generated file, lockfile, or remote state; `plan`/`apply` only see accepted concrete values. |
...
| 13 | AI inputs | Vision + short brief; seller-editable `prompts/seo.md`; hard proposal validation before the browser sees it. |
