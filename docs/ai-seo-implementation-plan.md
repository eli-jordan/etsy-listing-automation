...
| Drafting on design attach | Attaching a design to a saved listing whose brief is empty drafts a brief from the artwork, writes it through the ordinary autosave path, and then starts one SEO request (PRD 68). Only that transition arms it, only into an empty brief, once per attach, and never again automatically after a failure. The chain belongs to the open editor: it uses the same request-scoped endpoints, the same 60-second deadline per request, and the same abort-on-leave rule — there is still no server-side job, queue or proposal cache. |
